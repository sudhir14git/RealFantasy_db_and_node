...
let getAdminMargin = (connection) => {
    return new Promise((resolve, reject) => {
        let query = "SELECT rtype,amount FROM referalcommisions where rtype='adminmargin'";
        ...
        try {
           ...
           const newRankList = rankList.map(obj =>
            ...
            for(var i=minRank;i<=maxRank;i++){
                ...
                let rankAmtDistribute = newRankList.filter(item => (item.rank1 == i));
                ...
                if(rankAmtDistribute && rankAmtDistribute.length>0){
                    ...
                    //     let validPoolPriceDetail = poolPriceArr.filter(item => ((rankAmtDistribute[0].sno >= item.pmin) && (rankAmtDistribute[0].sno <= item.pmax)));
                    ...
                    //if(rankAmtDistribute.length>0){
                        ...
                        let validPoolPriceDetail = poolPriceArr.filter(item => ((minRankMulti <= item.pmin) && (maxRankMulti >= item.pmax)));
           ...
           await listSameRank.forEach(async(itemDeside)=>{
                //console.log("itemDeside---->>>",itemDeside)
                await itemDeside["rankAmtDistribute"].forEach(async(itemDetail)=>{
                    let vAmt=0;
                    if(itemDetail.pooltype==="PER"){
                        ...
                        //////NEW1 END//////
                         ...
                         vAmt =await itemDeside["sumAmt"]/(itemDeside["rankAmtDistribute"].length); //(validAmt.length > 0) ? validAmt[0].pamount : 0;
                    }
    ...
    let winBal = await connection.query(sql, true, async (error, results, fields) => {
        ...
        if (error) {
            ...
            if (payableAmount > 0) {
                ...
                if(MAILSYSTEM.poolpriceset_payWinTDS==true){
        ...
        (mysql_pool.mysql_pool).getConnection(async function (err, connection) {
            try {
                if (err) {
                    ...
                    let getCompleteMatch = "select matchid,matchname,mdate from matches where mstatus ='cm'";//matchid='cplt20_2020_g1'"
                    //console.log("-------CHANGE IT FOR LIVE--------FOR matchid='cplt20_2020_g1'-------------")
